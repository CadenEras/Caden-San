...
module.exports = new Command({
  ...
  async run(message, args, client) {
    ...
    try {
      ...
      if (!admin) {
        if (permission) return message.reply("I can't mute this user ! Maybe they have an higher role than you or than me !")
      }
      ...
      if (!muteRole) {
        ...
        message.guild.channels.cache.forEach(async (channel, id) => {
          await channel.permissionOverwrites.create(muteRole.id, {
            ...
            ADD_REACTIONS: false,
          }, 0).then(channel => console.log(`[ROLE EVENT] Time : ${client.readyAt}. Change successfully applied in <#${channel.id}> from ${message.guild.name} for ${muteRole.name} role !!!`))
        })

        message.channel.send('Mute role succesfully added ! Please check the permissions of the role !!! If you see something wrong, contact us via the support server !!!')
      
      }
      //end of create role
      

      if (member.roles.cache.some((role) => role.name === 'muted')) return message.reply('This user is already mute !')
