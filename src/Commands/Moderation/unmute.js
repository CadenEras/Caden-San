...
module.exports = new Command({
   ...
   async run(message, args, client) {
      try {
         if (!args[1]) return message.reply("Forgot how to use this command ? Try `c!help unmute` to see how it works.")
         ...
         if (!toUnmute.roles.cache.some((role) => role.name === "muted") && !toUnmute.roles.cache.some((role) => role.id === cMuteRole))
