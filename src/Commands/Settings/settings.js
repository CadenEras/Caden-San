  ...
  async run(message, args, client) {
    ...
    if(!args[1]) {
        let settingEmbed = new Discord.MessageEmbed()
            ...
            .setDescription(`See the current settings of your guild with me ! You can set a property with the comment "`)
        ...
        if (guildProfile.memberDefaultRoleId) {
            settingEmbed.addField(`Default Role for new member: `, `<@${guildProfile.memberDefaultRoleId}> (${guildProfile.memberDefaultRoleId})`)
        } else {
        ...
        if(!["prefix", "muteRoleId", "memberDefaultRoleId", "logChannelId"].includes(args[1])) return await message.channel.send('You need to give a valid property to change.')
        if ("prefix" === args[1]) {
